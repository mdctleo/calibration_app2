...
class StudyInformation(db.Model):
    ...
    biodiCsvRow = db.relationship('BiodiCsvRows', cascade="all, delete-orphan", backref=STUDY_INFORMATION_T_NAME)
    biodiCsvCompleteRow = db.relationship('BiodiCsvCompleteRows', cascade="all, delete-orphan", backref=STUDY_INFORMATION_T_NAME)
    ...
    def __init__(self, studyName, studyDate, researcherName, piName, isotope, chelator, vector, target, cellLine, tumorModel,
                 mouseStrain, radioPurity, comments, gammaCounter, runDateTime, gammaRunComments, protocolId):
...
class Mouse(db.Model):
    ...
    csvId = db.Column(MOUSE_INFORMATION_C_CSV_ID, db.Integer, db.ForeignKey(STUDY_INFORMATION_T_NAME + '.' + STUDY_INFORMATION_C_ID), primary_key=True, nullable=False)

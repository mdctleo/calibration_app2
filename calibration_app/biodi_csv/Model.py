...
class Protocol(db.Model):
    ...
    gammaCounterRun = db.relationship('GammaCounterRun', backref=db.backref(PROTOCOL_T_NAME, lazy='joined'), lazy=True)
...
class StudyInformation(db.Model):
    ...
    studyName = db.Column(STUDY_INFORMATION_C_STUDY_NAME, db.String(45), primary_key=True)
    ...
    gammaCounterRun = db.relationship('GammaCounterRun', cascade="all, delete-orphan", backref=db.backref(STUDY_INFORMATION_T_NAME, lazy='joined'), lazy=True)
...
class Mouse(db.Model):
    ...
    gammaCounterRun = db.Column(BIODI_CSV_ROW_C_GAMMA_COUNTER_RUN, db.Integer, db.ForeignKey(GAMMA_COUNTER_RUN_T_NAME + '.' + GAMMA_COUNTER_RUN_C_ID))
    ...
    preInjectionActivity = db.Column(MOUSE_INFORMATION_C_PRE_INJECTION_ACTIVITY, db.Float)
    postInjectionActivity = db.Column(MOUSE_INFORMATION_C_POST_INJECTION_ACTIVITY, db.Float)
    preInjectionMass = db.Column(MOUSE_INFORMATION_C_PRE_INJECTION_MASS, db.Float)
    posetInjectionMass = db.Column(MOUSE_INFORMATION_c_POST_INJECTION_MASS, db.Float)
...
class GammaCounterRun(db.Model):
    __tablename__ = GAMMA_COUNTER_RUN_T_NAME
    id = db.Column(GAMMA_COUNTER_RUN_C_ID, db.Integer, primary_key=True, autoincrement=True)
    ...
    runDateTime = db.Column(STUDY_INFORMATION_C_RUN_DATE_TIME, db.DateTime, nullable=False)
    gammaRunComments = db.Column(STUDY_INFORMATION_C_GAMMA_RUN_COMMENTS, db.Text)
    biodiCsvRows = db.relationship('BiodiCsvRow', cascade="all, delete-orphan", backref=db.backref(GAMMA_COUNTER_RUN_T_NAME, lazy='joined'), lazy=True)
    windows = db.relationship('Window', cascade="all, delete-orphan", backref=db.backref(GAMMA_COUNTER_RUN_T_NAME, lazy='joined'), lazy=True)
    mice = db.relationship('Mouse', cascade="all, delete-orphan", backref=db.backref(GAMMA_COUNTER_RUN_T_NAME, lazy='joined'), lazy=True)

class BiodiCsvRow(db.Model):
    __tablename__ = BIODI_CSV_ROW_T_NAME
    id = db.Column(BIODI_CSV_ROW_C_ID, db.Integer, primary_key=True, autoincrement=True)
    gammaCounterRun = db.Column(BIODI_CSV_ROW_C_GAMMA_COUNTER_RUN, db.Integer, db.ForeignKey(GAMMA_COUNTER_RUN_T_NAME + '.' + GAMMA_COUNTER_RUN_C_ID))
    rowNum = db.Column(BIODI_CSV_ROW_C_ROW_NUM, db.Integer, nullable=False)
    measurementTime = db.Column(BIODI_CSV_ROW_C_MEASUREMENT_DATETIME, db.DateTime)
    completionStatus = db.Column(BIODI_CSV_ROW_C_COMPLETION_STATUS, db.Integer)
    runId = db.Column(BIODI_CSV_ROW_C_RUN_ID, db.Integer)
    rack = db.Column(BIODI_CSV_ROW_C_RACK, db.Integer)
    det = db.Column(BIODI_CSV_ROW_C_DET, db.Integer)
    pos = db.Column(BIODI_CSV_ROW_C_POS, db.Integer)
    time = db.Column(BIODI_CSV_ROW_C_TIME, db.Float)
    sampleCode = db.Column(BIODI_CSV_ROW_C_SAMPLE_CODE, db.String(45))
